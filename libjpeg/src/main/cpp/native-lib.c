...
Java_com_andnux_libjpeg_JpegNative_compress(JNIEnv *env, jclass type, jobject bitmap, jint quality,
                                            jstring path_, jboolean optimize) {
    AndroidBitmapInfo androidBitmapInfo;
    BYTE *pixelsColor;
    int ret;
    BYTE *data;
    BYTE *tmpData;
    const char *dstFileName = (*env)->GetStringUTFChars(env,path_,JNI_FALSE);
    //解码Android Bitmap信息
    ...
    ALOGD("bitmap: width=%d,height=%d,size=%d , format=%d ",
